...
{
#if SPECIFY_NOEXCEPT
  inline void do_terminate_if(const bool do_throw_exception) noexcept
  {
    noexcept_benchmark::throw_exception_if(do_throw_exception);
  }
#else
  inline void do_terminate_if(const bool do_throw_exception)
  {
    try
    {
      noexcept_benchmark::throw_exception_if(do_throw_exception);
...
double LIB_NAME::test_explicitly_terminate_versus_noexcept()
{
  ...
  return noexcept_benchmark::profile_func_call([]
  {
    ...
    {
      do_terminate_if(volatile_false);
    }
  });
}
