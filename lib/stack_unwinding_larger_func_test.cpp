...
namespace
{
  ...
  void larger_recursive_func(recursion_data& data) OPTIONAL_EXCEPTION_SPECIFIER
  {
    if (--data.number_of_func_calls_to_do > 0)
    {
      noexcept_benchmark::throw_exception_if(data.volatile_false);
      object_class stack_object1(data.object_counter);
      noexcept_benchmark::throw_exception_if(data.volatile_false);
      object_class stack_object2(data.object_counter);
      larger_recursive_func(data);
    }
  }
}
...
double LIB_NAME::test_recursive_stack_unwinding_larger_func()
{
